...
Storyblok Data Cache：`unstable_cache` / `fetch.next.tags`，`NEXT_PUBLIC_STORYBLOK_REVALIDATE_TIME` 默认也是 **600**。  
**CMS 不直写 Redis**：内容先进 Next Data Cache，生产再由 cache-handler 落到带前缀的 Redis key。

主动失效链路（`app/api/storyblok/route.ts`）：

```text
Storyblok 发布 → POST /api/storyblok（验签）
  → revalidateTag(payload.full_slug)  // blog 目录用 sb-${BlogPage}；layout 额外 revalidatePath
  → Next → Handler.revalidateTag
  → 只删当前 keyPrefix（`${APPLICATION_ENV}-${buildId}:`）下带该 tag 的条目
  → 下次请求 miss → 再拉 Storyblok（常用新鲜 cv）→ 写回新缓存
```

要点：

- Webhook **只失效当前运行 build** 的命名空间；同环境多 Pod 共享同一前缀，一台删完大家下次都 miss。  
- **旧 build 的 Redis key 不会被 webhook 清**：新进程根本不读旧前缀；靠物理 TTL（~3600s）或淘汰自灭。  
- Storyblok CDN 的 `cv` 故意用新鲜值时，失效权威在 Next tag，不在 Storyblok CDN。

`instrumentation.js` 里 `registerInitialCache` **目前注释掉**——不靠启动灌全量，靠流量回填。

**面试回答版本**

"Handler 用 neshca 的 redis-strings。key 带环境和 buildId，发版天然隔离。操作超时 1 秒，超时当 miss，不拖死 TTFB。物理过期估 1 小时，业务 revalidate 多为 10 分钟。CMS 不直写 Redis：webhook 调 revalidateTag，删的是当前 build 前缀下带该 slug tag 的条目；旧 build 靠隔离加 TTL 自灭，不靠 webhook 扫全库。启动 connect 没有超时是已知限制。"
...
| 问题 | 项目侧做法 |
| --- | --- |
| 失效 | 事件优先：Storyblok webhook → `revalidateTag(full_slug)`（只打**当前** `env-buildId` 前缀）；时间兜底：`revalidate` + Redis 物理 expire |
| 发版脏缓存 | `keyPrefix` 含 `buildId`：新旧 build 读写隔离；旧 key **不**由新版本 webhook 清扫 |
...
"失效靠 webhook tag 优先、TTL 兜底。revalidateTag 只作用当前 build 的 Redis 前缀，多 Pod 共享所以一次就够。旧部署数据靠 buildId 隔离加物理过期自灭。防击穿主要靠物理过期长于业务 stale，以及请求内去重。雪崩靠错开过期和降级。搜索并发去重是客户端 Promise 单飞，跟 Redis 击穿是同一思想、不同层。"
...
| 只靠定时 revalidate | CMS 改完几分钟才生效 | webhook + revalidateTag（当前 build 前缀） |
| 误以为 webhook 清全库 | 期待旧 build key 立刻消失 | buildId 隔离；旧 key 靠 TTL，不扫历史前缀 |
...
发版隔离。新 build 读写新前缀；旧 key 靠 Redis TTL 自然淘汰，避免新代码读旧序列化形态。Webhook 也因此**只清当前 build**，不会（也不该）扫掉历史前缀。
...
### Q11. Storyblok webhook 之后，当前 build 的旧 CMS 缓存怎么失效？

CMS **不直写 Redis**。拉取时用 `unstable_cache` / `fetch` 挂上 `tags`（常见：`full_slug`、`sb-${contentType}`）。发布后 webhook 验签 → `revalidateTag(full_slug)`（blog 目录打 content-type tag；layout 额外 `revalidatePath`）→ Next 调 handler 删掉**当前** `${APPLICATION_ENV}-${buildId}:` 下带该 tag 的 key → 下次 miss 再拉 Storyblok 写回。同版本多 Pod 共享前缀，一台收 webhook 即可。

### Q12. 旧 build 的 Redis 数据谁清？webhook 会扫全库吗？

**不会。** 新部署的 `buildId` 变了，读写都在新前缀；旧前缀对新进程不可见。Webhook 的 `revalidateTag` 只作用当前 handler 的 `keyPrefix`。历史 key 靠物理 expire（估 ~3600s）或 Redis 淘汰。

### Q13. Storyblok webhook 和 Redis TTL 谁优先？

**主动 revalidateTag/Path 优先**（立刻删当前 build 下带 tag 的条目）；`revalidate` / Redis 物理 expire 是兜底。Storyblok CDN `cv` 常故意用新鲜值，失效权威在 Next tag，不在 CDN。

### Q14. 搜索 10s 和 Redis 1s 为何差这么多？
...
"我们缓存分两层别混。内容页多 Pod 场景下，Next 默认内存 ISR 不共享，生产用 neshca cacheHandler 接 Redis，key 带环境和 buildId，操作超时 1 秒，挂了降级 LRU；业务 revalidate 大约 10 分钟，Redis 物理过期估 1 小时，方便 stale-while-revalidate。CMS 不直写 Redis：webhook 调 revalidateTag，只删当前 build 前缀下带 slug tag 的条目；旧 build 靠隔离加 TTL 自灭。列表页 force-dynamic 不上这套。搜索是另一套：自定义 InstantSearch client 实现 responsesCache，Map 里存 Promise，2 分钟 TTL 加 in-flight 去重，解决 hydration 闪烁和 routing 双请求，数据不进 Redis。"
