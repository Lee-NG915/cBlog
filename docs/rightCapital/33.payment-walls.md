...
4. ActionSchema 分支处理：
     ...
     SDK_POPUP   → prepareSdkPayment 前置流程，SDK 弹窗交互后回调 capture（PayPal / Affirm）
     SUCCESS     → 直接进入第 5 步
5. capturePaymentAction()      Server Action — 闭环 capture
6. redirect → /checkout-success
```

注释参见 `payment-wallets.tsx` 顶部的流程注释。
...
> ⚠️ 错误分类详细规则、所有 provider code 归属，详见对照表：[payment-error-classification-table.md](payment-error-classification-table.md)
...
| `onAffirmSubmit`                      | Affirm：页面 submit 按钮 → `prepareSdkPayment` → `affirm.checkout()` 弹窗 |
| `onPaypalCapture` / `onAffirmCapture` | SDK 弹窗回调后的 capture                |
| `prepareSdkPayment`                   | PayPal / Affirm 的 SDK_POPUP 前置流程   |
| `onSdkError` / `onSdkCancel`          | SDK 异常 / 用户中止回调                 |
...
错误分类逻辑见 `utils/classify-payment-error.ts`（对照表见本目录 [payment-error-classification-table.md](payment-error-classification-table.md)）。
...
| **Affirm**         | Affirm JS SDK（`window.affirm.checkout`）    | `onAffirmSubmit` → SDK 弹窗 → `onAffirmCapture` | SDK_POPUP 模式，但提交入口是页面 submit 按钮（`showSubmitButton` 含 Affirm）；dismissal 走 `onSdkCancel` |
| **Stripe Express** | `ExpressCheckoutElement` (Apple/Google/Link) | `onExpressCheckoutSubmit`                     | Element 自带 submit 入口                                                |

`activeSubmitHandler`（`payment-wallets.tsx`）按 `isXxxSelected` 标志位选用对应 handler，并在调用前 dispatch `placeOrderClickedEvent`。
...
`paymentMethodClickCategory` / `placeOrderLabel` 根据 `paymentDataSource.source` + `orderInfo` 是否存在 + `resumeState?.status` 联合派生（`payment-wallets.tsx`）。
...
showSubmitButton={isStripeSelected || isGrabPaySelected || isTwoCTwoPSelected || isAffirmSelected}
submitLabel={
  orderInfo && orderPaymentsHasPending !== false
    ? `${t('makePayment')}${formattedCountdown ? ` (${formattedCountdown})` : ''}`
    : t('placeOrder')
}
```

- PayPal / Express 的提交入口在自己的 Element 上，因此 `showSubmitButton` 排除它们；Affirm 走页面 submit 按钮（`onAffirmSubmit`）
- 文案走 i18n（`makePayment` / `placeOrder`），根据是否已有订单 + 是否有 pending 支付切换
...
全量 31 个 category 及 provider code 归属详见本目录 [payment-error-classification-table.md](payment-error-classification-table.md)。常见 category：

- `orderExpired` / `orderCanceled` / `paymentSuccessOrderCanceled` — 订单状态类，走专属 modal 分支
- `orderAlreadyPaid` — 订单已支付
- `paypalError` — PayPal 专属（`{{shortMessage}}` 模板插值）
- `cardDeclinedError` — 卡被拒绝
- `paymentPending` — 支付处理中
- `paymentProcessingError` — PRD row 14 支付处理失败专属桶
- `genericPaymentError` — 最终兜底

### 8.3 Capture 重试

`utils/capture-with-retry.ts` 封装 capture 阶段的重试：每次尝试 5s 超时（`Promise.race`），最多重试 3 次（共 4 次尝试），重试间隔为指数退避 `1s → 2s → 4s`（cap 8s，无 jitter）。5s 内拿到明确响应立即返回不重试；非超时异常返回 `CAPTURE_FAILED` 不重试；全部超时后返回 `errorCode: 'processing'`。防止网络抖动导致最后一步失败、用户已扣款无法闭环。详细策略与 jitter 预案见 [known-gaps.md](known-gaps.md) §3。
...
│   ├── payment-button-skeleton.tsx          按钮骨架屏
...
│   ├── use-payment-execution.spec.ts        单测
