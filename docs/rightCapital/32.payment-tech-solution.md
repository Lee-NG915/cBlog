...
- `PaymentStrategyFactory` 落在 `infrastructure` 层
...
| GrabPay     | `REDIRECT`             | 跳转已支持；回调统一走 `/checkout/payment-callback`（reconcile 轮询）                     | 已贯通   |
| 2C2P        | `REDIRECT`             | 页面预留 confirm handler 但 SDK 未接线；回调走 `payment-callback`（TCTP channel，待联调） | 未闭环   |
| ZipPay      | `REDIRECT`             | 回调统一走 `/checkout/payment-callback`（reconcile 轮询）                                 | 已贯通   |
| PayPal      | `SDK_POPUP`            | SDK 弹窗接入，`prepareSdkPayment()` 协议已对齐                                            | 已贯通   |
| Affirm      | `SDK_POPUP`            | 同上                                                                                      | 已贯通   |

### 5.3 当前架构约束与待对齐项

`PaypalStrategy` / `AffirmStrategy` 均返回 `SDK_POPUP`，与组件层协议一致；REDIRECT 型回调路由统一收口到 `/checkout/payment-callback`。

当前待对齐项（详见 [known-gaps.md](./known-gaps.md)）：

- **2C2P 端到端未闭环**：`twoCTwoPConfirmHandlerRef` 为占位未赋值（2C2P 尚未真正集成调试，仅预留方法），回调 reconcile 路径待联调
...
`captureWithRetry()`（`payment/components/.../payment-wallets/utils/capture-with-retry.ts`）让支付闭环确认具备有限重试能力，降低由于网络抖动、瞬时后端异常造成的“支付已完成但前端感知失败”问题。关联 Sentry `JOYBOY-WEB-4JV`。

**分层归属**：capture 的超时 / 重试**唯一**由客户端 `captureWithRetry` 掌控；内层 fetch（`requestToCapturePayment`）只用 `30s` socket 兜底 + `retries=0`，且该兜底必须 **> 外层单次超时**，否则内层会先超时并把结果吞成 resolved 失败，从而遮蔽外层重试。

**参数**：

- per-attempt 超时 `5s`，最多重试 `3` 次（共 4 次尝试）。
- 重试间隔为指数退避 `min(1s × 2^attempt, 8s)` = `1s → 2s → 4s`（无 jitter；低 QPS + 后端按 idempotencyKey 去重下的取舍，加 jitter 的方式见文件内注释）。
- 最坏总耗时 ≈ `4 × 5s + (1+2+4)s = 27s`。

**判定规则**：

- 5s 内拿到**明确响应**（成功 / 后端明确失败）→ 立即返回，不重试。
- 抛出**非超时异常** → 返回 `{ success:false, errorCode:'CAPTURE_FAILED' }`，不重试。
- 5s 超时（`CAPTURE_TIMEOUT`）→ 未满 3 次则退避后重试；满 4 次返回 `{ success:false, errorCode:'processing' }`。

**幂等**：`idempotencyKey` 由 action 层 `?? traceId` 提供单一稳定来源，6 个策略 capture 路径直接透传 `params`，重试复用同 key、后端按 key 去重防重复扣款。

```mermaid
flowchart TD
    Start(["captureWithRetry(params)"]) --> Init["attempt = 0"]
    Init --> Race{"Promise.race:<br/>capturePaymentAction  vs  5s 超时"}

    Race -->|"5s 内返回<br/>成功 / 后端明确失败"| Def[["返回该结果<br/>（definitive，不重试）"]]
    Race -->|"非超时异常 throw"| Fail[["返回 success:false<br/>errorCode: CAPTURE_FAILED<br/>（不重试）"]]
    Race -->|"5s 超时<br/>CAPTURE_TIMEOUT"| Check{"attempt &lt; 3 ?"}

    Check -->|"是"| Backoff["指数退避等待<br/>min(1s × 2^attempt, 8s)<br/>→ 1s / 2s / 4s"]
    Backoff --> Inc["attempt++"]
    Inc --> Race
    Check -->|"否（已 4 次）"| Proc[["返回 success:false<br/>errorCode: processing"]]
```
...
4. **风险点识别**
   识别页面层与 Strategy 层之间的协议不一致、层间职责漂移等风险点。
...
Service 与 Strategy 已为 2C2P 定义统一入口（REDIRECT 型），但组件层 confirm handler 为占位。2C2P 尚未真正集成调试，待接入时一并补齐组件层接线与 `payment-callback` reconcile 联调。

#### 2. REDIRECT 回调真实场景待联调

`payment-callback` 的 `CallbackChannel`（SPL / GRAB_PAY / ZIP_PAY / TCTP）在各 provider 真实回跳场景下的 reconcile 路径需联调验证。
