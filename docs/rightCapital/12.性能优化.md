...
- **加载与渲染类**
  - `FCP`：用户第一次看到内容的时间。
  - `LCP`：首屏主要内容真正出来的时间。
  - `CLS`：专门用来衡量网页在加载和浏览过程中，页面元素发生“意外”位移的程度，以评估网页的视觉稳定性。
    良好（Good）：≤ 0.1需要改进（Needs Improvement）：0.1 - 0.25差（Poor）：> 0.25
  - `INP`：整个页面生命周期中，用户单次交互的响应速度，页面多久真正响应。
  - `TTI`：页面加载阶段，页面整体可交互的时间点 ，老一点的口径，有些老文档还会看。
...
1. Sentry + 自研 Web Vitals 上报（CLS 精确定位）
