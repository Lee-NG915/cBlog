...
4. `MIGRATION_ASSET_DIR` 使用 Git worktree 外的绝对路径指向迁移对象暂存目录；Phase 3 后由 S3 compatible adapter 取代。
...
门禁：plan 中 `unresolvedAssets` 必须为空；apply 和 verify 的 source digest 必须来自相同配置；verify 必须返回 `ok: true`。任何差异都禁止进入切流。
...
- 内容写入冻结，保存 PostgreSQL 原生备份并完成一次隔离 `pg_restore`。
- 将 filesystem asset adapter 替换为生产 S3 adapter，并校验所有对象 SHA-256。
