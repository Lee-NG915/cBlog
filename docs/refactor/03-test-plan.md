...
## 6. 执行记录（2026-08-14 重构实施）

实际执行方式与计划的差异及结果：

- **自动化单测（Vitest）**：28 个用例全过，覆盖 CORE-001~010 全部、MIG-003/004、ADM-004/005 后端部分（frontmatter round-trip/字节稳定、slug 与路径穿越、导入幂等与同步、漂移检测、保存回滚触发器测试、软删除回收站、排序回写、保留字、图片重名 hash）。
- **等价校验（MIG-001/002）**：`scripts/parity-snapshot.mjs` 在 Phase 1/2/3/5 各执行一次，76 页 URL、26 项 sitemap、每页指纹（title/h1/站内链接序列）全程与基线一致；专栏 slug 44/44 对上。
- **管理端 E2E**：以 curl 驱动 API 全链路代替 Playwright（单作者项目务实取舍，Playwright 基建列 backlog）：新建→保存回写→贴图落盘→状态双写→发布白名单分类→分类/专栏 CRUD→排序回写→软删除，全部通过后清理测试数据并复原数据库。
- **浏览器目检**：仪表盘/列表/编辑器渲染正常，编辑器预览 Mermaid 渲染成功（1 图例→SVG）；查看器打开/关闭/ARIA/滚动锁定经真实事件验证。
- **环境限制记录**：内嵌浏览器面板隐藏时 IntersectionObserver 与 requestAnimationFrame 挂起（已用探针证实），Mermaid 懒渲染触发与缩放 transform 的像素级表现无法在该环境截图验证，改以代码审查 + DOM 断言覆盖；建议在真实浏览器过一遍 §5 回归清单第 2、5 条（MMD-001~006 交互手测）。
- **PERF-001**：基线 worktree 对照构建，文章页 first-load 106→103 kB，共享包 88.4→88.5 kB，详见 [perf-report](./perf-report.md)。
- **PUB-005（真实发布）**：未执行——发布上线属外部动作，由作者首次使用时验证（管理端发布页已具备全部护栏）。
