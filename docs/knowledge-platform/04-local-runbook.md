...
### 验收 Addx 与 RightCapital

刷新工作台后打开「项目」：Addx 应有 19 篇、RightCapital 应有 28 篇，顺序与原专栏一致。
打开任一笔记，通过正文上方「学习路径」进入新的学习顺序；通过「项目」回到原专题。
也可在侧栏「学习路径」直接打开对应路径，并用上一篇、下一篇连续阅读。
同一笔记在主题、项目和路径中共用正文，修改一次即生效；所有迁移笔记保持私有。

初始路径与分类配置位于 `scripts/knowledge/collection-seeds.mjs`，仅用于迁移初始化。
在工作台调整成员、顺序或名称后，重复导入不会覆盖这些更改。
