...
- 目标平台：Sentry + Grafana
- 重要调整：不再使用 Datadog Logs / Datadog RUM；Grafana 接管 dashboard、logs query、metrics、traces 与 alerting 入口。
- 相关现状文件：
...
交易链路是 Joyboy 最关键的业务闭环。Sentry 已完成平台级初始化，`libs/shared/observability` 已有统一 logger 与错误捕获封装。原方案中 Datadog Logs / Datadog RUM 的职责将迁移到 Grafana 体系：
...
交易域不再设计 Datadog RUM action。统一使用 transaction observability helper 输出交易事件，由平台侧转换为：
...
- `trackTransactionStep(step, result, context)`
- `trackTransactionTiming(step, durationMs, context)`
- `captureTransactionError(error, context)`
...
建议继续使用交易可观测性封装，而不是在业务里直接散写 Sentry / logger / metrics 调用：
...
结合“Datadog Logs / RUM 不再使用”的新方向，优先做以下几件事：

1. 保留 Sentry 交易高采样与错误分桶。
2. 将现有 Datadog transaction helper 迁移为通用 transaction event helper。
