...
| 常量 | 标签值 | 自动推断 priority | 自动推断 severity |
|------|--------|-------------------|-------------------|
| `BUSINESS_DOMAIN.PAYMENT` | `payment` | high | fatal |
| `BUSINESS_DOMAIN.CHECKOUT` | `checkout` | high | fatal |
| `BUSINESS_DOMAIN.USER` | `user` | high | fatal |
| `BUSINESS_DOMAIN.ORDER` | `order` | high | error |
| `BUSINESS_DOMAIN.CART` | `cart` | medium | warning |
| `BUSINESS_DOMAIN.PRODUCT` | `product` | medium | warning |
| `BUSINESS_DOMAIN.SEARCH` | `search` | medium | warning |
| `BUSINESS_DOMAIN.PROMOTION` | `promotion` | medium | warning |
| `BUSINESS_DOMAIN.WISHLIST` | `wishlist` | low | warning |
| `BUSINESS_DOMAIN.CMS` | `cms` | low | warning |
...
**自动推断的标签：** `domain: payment` · `priority: high` · `level: fatal`
...
**自动推断的标签：** `domain: checkout` · `priority: high` · `level: fatal`
...
**自动推断的标签：** `domain: cart` · `priority: medium`
...
**自动推断的标签：** `domain: product` · `priority: medium`
...
**自动推断的标签：** `domain: user` · `priority: high` · `level: fatal`
...
**自动推断的标签：** `domain: cms` · `priority: low`
...
**自动推断的标签：** `domain: search` · `priority: medium`
...
### 8. error_bucket 分类（beforeSend 自动推断）
...
在 `libs/shared/observability/src/lib/sentry/contexts/domains.ts` 中添加新常量，并在 `libs/shared/observability/src/lib/sentry/contexts/priorities.ts` 中配置对应的优先级规则。
...
| 标签 | 可能的值 | 来源 |
|------|----------|------|
| `domain` | payment, checkout, cart, product, user, order, cms, search, wishlist, promotion | 调用 `captureStructuredError` 时显式传入 |
