...
export default class Finger extends Component {
  ...
  static getAngle(v1, v2) {
    const mr = Finger.getLen(v1) * Finger.getLen(v2);
    if (mr === 0) return 0;
    let r = Finger.dot(v1, v2) / mr;
    if (r > 1) r = 1;
    return Math.acos(r);
  }
  ...
  static getRotateAngle(v1, v2) {
    let angle = Finger.getAngle(v1, v2);
    if (Finger.cross(v1, v2) > 0) {
      angle *= -1;
    }
  ...
  _handleTouchStart(evt) {
    ...
    if (len > 1) {
      ...
      preV.x = v.x;
      preV.y = v.y;
      this.pinchStartLen = Finger.getLen(preV);
    ...
    if (len > 1) {
      ...
      if (preV.x !== null) {
        if (this.pinchStartLen > 0) {
          ...
          evt.scale = Finger.getLen(v) / this.pinchStartLen;
          evt.zoom = evt.scale;
          this._emitEvent('onPinch', evt);
        }
        evt.angle = Finger.getRotateAngle(v, preV);
