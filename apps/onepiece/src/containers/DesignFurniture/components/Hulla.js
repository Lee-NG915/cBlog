    ...
    if (!document.getElementById('hintegrate')) {
      ...
      script.onload = () => {
        ...
        if (window?.HIntegrate) {
          window.HIntegrate?.initialiseExperience(folder, '#hulla-configurator');
        }
      };
    } else {
      setLoaded(true);
      ...
      for (const item of items) {
        ...
        try {
          await dispatch(
            ...
            failedItems.push({ id: product?.ident, name: product?.name, error: error?.errors?.[0]?.detail || error });
          });
        } catch (error) {
          console.log(error);
        }
