...
export async function generateStaticParams() {
  return getAllAddxAiSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({
  params,
}: AddxAiNotePageProps): Promise<Metadata> {
  const slug = decodePathSegment(params.slug);
  const note = getAddxAiNoteBySlug(slug);
...
export default async function AddxAiNotePage({ params }: AddxAiNotePageProps) {
  const slug = decodePathSegment(params.slug);
  const note = getAddxAiNoteBySlug(slug);
  ...
  const allNotes = getAllAddxAiNotes();
    ...
    <>
      ...
      <article className="min-w-0">
        <div data-reveal="fade">
          <BackButton href="/addx-ai" label="返回笔记目录" />
        </div>
