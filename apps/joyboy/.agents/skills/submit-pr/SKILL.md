...
name: submit-pr
description: >
  Use when the user requests "submit PR", "create PR", "open a PR",
  "raise a PR for current branch", or similar. Creates or updates a GitHub PR
  following conventions: parses ClickUp task ID from branch name, auto-fills
  "related pr & ticket" with clickable links, and generates a title in the
  format (feat/fix + [#<task_id>] + task title).
---
...
## Observability Override (Sentry auto-resolve)

When resolved Sentry issue IDs are provided or detected in the current task context (for example: `JOYBOY-WEB-32S`), follow `AGENTS.md` Sentry auto-resolve rules automatically:

Deterministic activation rule:

- Activate override only when **resolved Sentry issue IDs list is non-empty**.
- ID source priority:
  1. Explicit resolved list from the caller/task workflow (for example Step 9 report inputs).
  2. Explicit IDs provided in the current user instruction.
- Do not activate override from ambiguous free-text mentions without resolved intent.

- Keep ClickUp ticket/branch context in the PR body as usual.
- Override title suffix to Sentry format:
  - `...[fixes JOYBOY-WEB-XXXX]`
  - `...[fixes JOYBOY-WEB-XXXX, fixes JOYBOY-WEB-YYYY]`
- Add one line per issue in PR description:
  - `fixes JOYBOY-WEB-XXXX`
- Do not place `fixes ...` in commit messages.
- If generated title/body does not satisfy these rules, run `gh pr edit` to correct before returning.
...
- If Observability Override is active: append Sentry suffix from resolved issue IDs, e.g.
  - `fix(observability): auto-fix Sentry classification violations [fixes JOYBOY-WEB-XXXX]`
  - `fix(observability): auto-fix Sentry classification violations [fixes JOYBOY-WEB-XXXX, fixes JOYBOY-WEB-YYYY]`
...
- If Observability Override is active, validate title/body after create/update and immediately correct with `gh pr edit` when needed.
