...
import { useUpdateZipcodeInCartMutation } from '@castlery/modules-cart-domain';
...
export function SearchPlaces({ type, afterSubmit }: SearchPlacesProps) {
  // 合并相关状态
  ...
  const [updateZipcodeInCart] = useUpdateZipcodeInCartMutation();
  ...
  const enabledToSearch = useMemo(() => {
    ...
    return type === SearchPlacesType.ZIPCODE
    ...
    async (value: string): Promise<GoogleAddressOption[]> => {
      ...
      try {
        const res = await searchTrigger({
          ...
          type: type === SearchPlacesType.ZIPCODE ? type : '',
        });
  ...
  // 更新购物车中的邮编
  const handleUpdateZipcodeInCart = useCallback(async () => {
    ...
    try {
      ...
      const res = await updateZipcodeInCart({
      ...
      if (res?.error) {
        const errorMsg = 'message' in res.error ? JSON.stringify(res.error.message) : JSON.stringify(res.error);
  ...
  }, [errorMessage, searchState, updateZipcodeInCart, updateLoadingState, updateErrorState, afterSubmit]);
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (type === SearchPlacesType.ZIPCODE) {
        handleUpdateZipcodeInCart();
      } else {
        ...
        afterSubmit?.(formattedAddress);
      }
    },
    [type, searchState, handleUpdateZipcodeInCart, formatAddressData, afterSubmit, updateErrorState]
  );
