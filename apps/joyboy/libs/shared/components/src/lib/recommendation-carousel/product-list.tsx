...
import { ProductItem, ProductItemDataProps } from './product-item-adapter';
...
import { AtcActionToast } from '../atc-action-toast/atc-action-toast';
...
const ProductList = ({
  ...
  const [atcToastOpen, setAtcToastOpen] = useState(false);
  const [atcToastStatus, setAtcToastStatus] = useState<'success' | 'error'>();
  ...
  const handleAtcToastOpen = (success: boolean) => {
    setAtcToastOpen(true);
    setAtcToastStatus(success ? 'success' : 'error');
  };
  ...
  if (forceGridDisplay || (!needSliderDisplay && desktop)) {
    return (
      ...
      >
          ...
          return (
            ...
            >
              <ProductItem
                ...
                onAtcToastOpen={handleAtcToastOpen}
              />
            </Stack>
          );
        })}
        ...
        <AtcActionToast open={atcToastOpen} onClose={() => setAtcToastOpen(false)} status={atcToastStatus} />
      </Stack>
    );
  }
  return (
