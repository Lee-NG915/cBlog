 ...
 * payment on the Affirm site. Not yet implemented.
 */
const paymentCallbackUrl = `${EcEnv.NEXT_PUBLIC_WEB_SERVER_NAME}/${EcEnv.NEXT_PUBLIC_COUNTRY.toLowerCase()}${
  basePageConfig['checkout-success'] // TODO: replace with /checkout/payment/callback
}`;

/**
 * Handles Affirm buy-now-pay-later payments (US market).
 * Flow: initiatePaymentIntent → REDIRECT to Affirm authorization page → capture on return.
 *
 * The Affirm config (cancelUrl, returnUrl, platform) is constructed by the component
 * layer before calling initiatePaymentAction and forwarded via paymentConfig.affirmConfig.
 */
export class AffirmStrategy implements IPaymentStrategy, IConfirmPayment {
  readonly type = PaymentMethodProviderEnum.AFFIRM;
      ...
      const res = await requestToInitializePayment(
        {
          ...
          paymentConfig: cmd.config.affirmConfig ? { affirmConfig: cmd.config.affirmConfig } : undefined,
        },
        this.traceId
      );
      if (res?.code !== 0) {
  ...
  async confirmPayment(intentResult: InitiatePaymentIntentResult): Promise<ActionSchema> {
    ...
    if (!redirectUrl) throw new Error('Missing redirectUrl in Affirm intent result');
