...
}/${EcEnv.NEXT_PUBLIC_COUNTRY.toLowerCase()}${'/checkout/zippay'}`;

/**
 * Handles ZipPay payments (AU market).
 * Flow: initiatePaymentIntent → REDIRECT to Zip authorization page → capture on return.
 */
export class ZipPayStrategy implements IPaymentStrategy, IConfirmPayment {
  readonly type = PaymentMethodProviderEnum.ZIPPAY;
      ...
      const res = await requestToInitializePayment(
        {
          ...
          paymentConfig: cmd.config.zipPayConfig ? { zipPayConfig: cmd.config.zipPayConfig } : undefined,
        },
        this.traceId
      );
      if (res?.code !== 0) {
  ...
  async confirmPayment(intentResult: InitiatePaymentIntentResult): Promise<ActionSchema> {
    ...
    if (!redirectUrl) throw new Error('Missing redirectUrl in ZipPay intent result');
