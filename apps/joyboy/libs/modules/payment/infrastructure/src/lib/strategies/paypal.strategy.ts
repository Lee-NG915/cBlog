 ...
 * payment on the PayPal site. This page reads URL params and triggers
 * capturePaymentAction. Not yet implemented.
 */
const paymentCallbackUrl = `${EcEnv.NEXT_PUBLIC_WEB_SERVER_NAME}/${EcEnv.NEXT_PUBLIC_COUNTRY.toLowerCase()}${
  basePageConfig['checkout-success'] // TODO: replace with dedicated /checkout/payment/callback route
}`;

export class PaypalStrategy implements IPaymentStrategy, IConfirmPayment {
  readonly type = PaymentMethodProviderEnum.PAYPAL_ONLINE;
  ...
  async initiatePaymentIntent(cmd: InitiatePaymentCommand): Promise<InitiatePaymentIntentResult> {
    ...
    try {
      const res = await requestToInitializePayment(
        {
          ...
          paymentConfig: cmd.config.paypalConfig ? { paypalConfig: cmd.config.paypalConfig } : undefined,
        },
        this.traceId
      );
      if (res?.code !== 0) {
      ...
      trackStrategyInitiateSuccess(cmd, {
        ...
        actionType: 'SDK_POPUP',
      });
      const paypalResult = initResult?.paymentResult?.paypalResult;
      if (!paypalResult) {
        return {
          success: false,
          error: { code: 'INITIATE_FAILED', message: 'Missing PayPal result in initiate payment response' },
        };
      }
      return {
        success: true,
  ...
  /**
   * Returns SDK_POPUP so the Component navigates the user to the PayPal
   * authorization page.
   */
  async confirmPayment(intentResult: InitiatePaymentIntentResult): Promise<ActionSchema> {
    ...
    const paypalResult = intentResult.data.paymentResult.paypalResult;
    if (!paypalResult) {
      throw new Error('Missing PayPal result in initiate payment response');
    }
    return {
      action: 'SDK_POPUP',
      sdkToken: paypalResult?.paypalOrderId ?? '',
