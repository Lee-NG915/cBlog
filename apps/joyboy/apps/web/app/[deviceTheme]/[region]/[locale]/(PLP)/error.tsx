 ...
 */
export default function PLPError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <PageErrorBoundary
      error={error}
      reset={reset}
      pageType={PAGE_TYPES.PLP}
      errorBoundaryName="PLPErrorBoundary"
      context={{ domain: BUSINESS_DOMAIN.SEARCH }}
    />
  );
}
