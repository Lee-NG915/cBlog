 ...
 */
export default function PLAError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <PageErrorBoundary
      error={error}
      reset={reset}
      pageType={PAGE_TYPES.PDP}
      errorBoundaryName="PLAErrorBoundary"
      context={{ domain: BUSINESS_DOMAIN.PRODUCT }}
    />
  );
}
