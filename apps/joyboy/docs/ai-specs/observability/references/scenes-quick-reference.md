...
- **Sentry effect**: `SentryContextProvider` calls `setGlobalSentryContext` in its render body (SSR). However, layout and page RSC execution order is **not guaranteed** under React concurrent rendering — **page.tsx must also call `setGlobalSentryContext` independently**.
- **Async layout exception**: If the layout has `await` operations that can throw a 500, add `setGlobalSentryContext` at the top of the layout function body, **before** the first `await`. This ensures that if the async operation fails, Sentry captures with correct tags. Example: `(shop-the-look)/shop-the-look/layout.tsx`. Pure JSX synchronous layouts do **not** need `setGlobalSentryContext` in the function body.
- **Layout error boundary**: Layout errors bubble to the **parent** segment's `error.tsx`, not the same-level one. `setGlobalSentryContext` called before the throw will have already written to isolation scope, so Sentry captures with context.
...
- **Steps**: Call `setGlobalSentryContext({ pageType, domain, priority, ... })` at the very top of the page function — **required even if the parent layout has `SentryContextProvider`**. `setGlobalSentryContext` writes to `getIsolationScope()` (per-request shared via AsyncLocalStorage), so subsequent Server Actions, server-side fetches, and Redux listeners in the same request inherit these tags.
- **Why isolation scope**: `getCurrentScope()` is per-async-context (isolated per RSC), so tags written in layout are invisible in page. `getIsolationScope()` is shared across all RSCs in the same HTTP request via AsyncLocalStorage inheritance — the correct scope for cross-RSC tag propagation.
...
- **Import**: `import { withServerActionInstrumentation, captureStructuredError, BUSINESS_DOMAIN, BusinessSeverity } from '@castlery/observability/server';`
- **Steps**:
  - **Throw path** (preferred): Wrap the action with `withServerActionInstrumentation(action, { actionName, monitoringContext: { domain, severity? }, recordSuccess?, context? })`. The wrapper catches errors, reports to Sentry, and re-throws. Do not write bare try/catch Server Actions without Sentry reporting.
  - **Graceful-degradation path** (return `{}` instead of throw): `withServerActionInstrumentation` always re-throws, so use `captureStructuredError` inside the catch block instead. Example: third-party non-critical services (DY campaigns) where callers expect an empty result on failure.
  - Old action files without either pattern are legacy awaiting migration.
- **Sentry effect**: Carries `error_bucket`, `domain` (from `monitoringContext.domain`), `priority`, etc.; `withServerActionInstrumentation` also attaches a Sentry action span.
...
- **Full MonitoringContext** (applies to all capture APIs — `captureStructuredError`, `captureStructuredMessage`, domain shortcuts): all fields in `MonitoringContext` are supported end-to-end via `withSentryContext` → `setSentryContext`:
...
## Project-Level error_bucket Implementation (Web Client — Browser)
...
- **What `beforeSend` actually does** (in order):
  ...
  3. For critical third-party errors (DY, Algolia, Stripe, PayPal) with no own code: keeps them and sets `event.tags.criticalThirdParty = 'yes'`
...
## Project-Level error_bucket Implementation (Web Server — Node.js)

- **File**: `apps/web/sentry.server.config.ts` — `beforeSend` inside `Sentry.init()`.
- **Classifier**: `classifyErrorBucketServer` from `@castlery/observability/server`. Export path: `libs/shared/observability/src/server.ts` → `libs/shared/observability/src/lib/sentry/errors/error-bucket.ts`.
- **Applies to**: Server Actions, API Routes — anything running in Node.js runtime. Does **not** apply to Edge runtime (middleware — known gap, `sentry.edge.config.ts` has no `beforeSend`).
- **Bucket set**: `upstream_5xx`, `upstream_timeout`, `js_fatal`, `unclassified`. Skips `hydration`/`third_party`.
  - `upstream_5xx`: statusCode 500-599 OR message matches 5xx pattern — our server called upstream and got 5xx
  - `upstream_timeout`: `AbortError` name OR message matches timeout/ETIMEDOUT
  - `js_fatal`: TypeError / ReferenceError / RangeError / SyntaxError
  - `unclassified`: everything else
- **No filter rules**: Unlike client `beforeSend`, server version does not noise-filter — all events pass through.
- **Alert rules**: `[Sev-2] Upstream Errors` (`error_bucket:[upstream_5xx,upstream_timeout] !bucket_confidence:low`), new issue OR >20/15min.
...
| `page_type`         | PAGE_TYPES; injected by `SentryContextProvider`'s `pageType`. **注意**：搜索页 (`/search`) 使用 `PAGE_TYPES.PLP`（`domain:search`），纳入 Sev-1 PLP 告警覆盖（2026-03-18）。                                                                                                                                                                                                                                |
