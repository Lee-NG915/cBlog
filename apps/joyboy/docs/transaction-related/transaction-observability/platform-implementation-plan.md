...
- Datadog Logs 已下线。
- Datadog RUM 已下线（`transaction-observability` 模块内的 `@datadog/browser-rum` 依赖已移除）。
...
2. ~~再迁移 transaction event helper，避免继续绑定 Datadog RUM。~~（已完成，详见 §3 P0-2）
3. 再建 Dashboard。
4. 再建 Alert Rules。
5. 再建 SLO / Burn Rate。
...
### P0-2 Transaction Event Helper 迁移（已完成）
...
1. ✅ `libs/shared/observability/src/lib/transaction-observability/datadog.ts` 已删除；模块内不再 `import '@datadog/browser-rum'`。
2. ✅ 业务 API 名称保持稳定（`trackTransactionStart/Success/Failure/Timeout`、`captureTransactionError`、`reportTransactionMessage`、`addTransactionBreadcrumb`），无业务代码大面积返工。
3. ✅ 事件经 `captureStructuredError` / `captureStructuredMessage` 进入 Sentry，同时由 `logTransactionEvent` 写入统一 logger；metrics / traces 由平台侧从 logs/Sentry 派生。
4. ✅ 设计层移除对 Datadog RUM `addAction` / `addError` / `addTiming` 的依赖；`TransactionEventOptions.sendToDatadog` 字段移除。

> 应用层 `apps/web/app/datadog-init.tsx`、`apps/pos/app/datadog-init.tsx` 等 Datadog 初始化文件不在本任务范围内，由应用层下线任务单独处理。
