...
> 本方案不使用 Datadog Logs / Datadog RUM。客户端关键交易事件已统一进入 `transaction-observability` helper（`trackTransactionStart/Success/Failure/Timeout`、`reportTransactionMessage`、`captureTransactionError`），由平台侧转换为 metrics / logs / traces，不再依赖 RUM action。
