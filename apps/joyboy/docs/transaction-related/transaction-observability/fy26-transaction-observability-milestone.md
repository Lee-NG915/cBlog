...
- 进展（2026-05-26）：`libs/shared/observability/src/lib/transaction-observability/` 模块已完成 Datadog 依赖下线；Grafana dashboards / alerts / SLO 仍在 P0 待办
...
- [x] Datadog Logs / RUM are removed from transaction monitoring code paths; no new Datadog metrics added.
  > Note: app-level `apps/web/app/datadog-init.tsx` / `apps/pos/app/datadog-init.tsx` remain as separate cleanup outside this milestone.
...
- [x] 交易监控代码路径中 Datadog Logs / RUM 已移除；不再新增 Datadog 指标。
  > 备注：应用层 `apps/web/app/datadog-init.tsx` / `apps/pos/app/datadog-init.tsx` 不在本 milestone，由后续应用层下线任务统一处理。
