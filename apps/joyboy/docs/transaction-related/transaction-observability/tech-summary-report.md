...
交易链路是 Joyboy 核心的业务闭环。新方案已将原 Datadog Logs / Datadog RUM 的职责迁移到 Grafana 体系（`transaction-observability` 模块已下线 `@datadog/browser-rum` 依赖）：
...
| Sentry 交易 helper（`captureTransactionError`、`reportTransactionMessage`、`addTransactionBreadcrumb`、`trackTransactionStart/Success/Failure/Timeout`）   | 完成 |
| 下线 transaction-observability 内的 Datadog RUM 集成（删除 `datadog.ts`、移除 `sendToDatadog` / `setTransactionRumContext` / `trackTransactionAction` 等） | 完成 |
...
3. 应用层 `apps/web/app/datadog-init.tsx` / `apps/pos/app/datadog-init.tsx` 等残留 Datadog 初始化文件的下线时间点（本 milestone 不包含）。
