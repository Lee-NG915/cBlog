...
1. 保留统一字段与步骤模型。
2. ~~将交易观测出口从 Datadog RUM helper 重构为通用 transaction event helper~~（已完成：`libs/shared/observability/src/lib/transaction-observability/` 不再依赖 `@datadog/browser-rum`，统一走 Sentry helper + logger）。
...
- [x] Datadog RUM helper 已下线（`datadog.ts` 删除，`sendToDatadog` / `setTransactionRumContext` / `trackTransactionAction` / `trackTransactionRumError` / `trackTransactionTiming` 全部移除）
...
- `libs/shared/observability/src/lib/transaction-observability/context.ts`

> Grafana-oriented 出口（Loki labels、Prometheus metrics、Tempo span attributes）由平台侧承接，模块内不再单独维护 `grafana.ts` / `datadog.ts`。

### 3.3 Datadog RUM helper 迁移（已完成）

Datadog RUM 不再作为交易监控依赖。模块内的 Datadog 集成已下线：

- [x] `libs/shared/observability/src/lib/transaction-observability/datadog.ts` 已删除。
- [x] `trackTransactionAction` / `trackTransactionRumError` / `trackTransactionTiming` / `setTransactionRumContext` 全部移除。
- [x] `TransactionEventOptions.sendToDatadog` 字段移除。
- [x] 所有交易事件改由 `captureTransactionException` / `captureTransactionMessage` 进入 Sentry，并通过 `logTransactionEvent` 写入统一 logger。
- [x] 业务层调用点（`use-payment-execution.ts`、`apps/web/.../checkout/payment/page.client.tsx`）的 `setTransactionRumContext` 调用已清理。

> 应用层 `apps/web/app/datadog-init.tsx` / `apps/pos/app/datadog-init.tsx` 不在本次范围内，由后续应用层下线任务统一处理。
...
- [x] 迁移 Datadog RUM helper 为通用 transaction event helper（已下线 Datadog 依赖）
