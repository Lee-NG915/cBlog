...
10 个 bucket，客户端 8 个 + 服务端专属 2 个（先匹配先赢）：

- 客户端：`hydration → third_party → network → api_5xx → api_timeout → app_error → js_fatal → unclassified`
- 服务端：`upstream_5xx → upstream_timeout → js_fatal → app_error → unclassified`  
  （注意：服务端是 **js_fatal 先于 app_error**，和客户端相反；以 `error-bucket.ts` 为准。）

#### 客户端怎么区分 network / api_5xx / api_timeout / app_error

靠的是**信号类型**，不是业务域名：

| Bucket | 人话语义 | 判定信号 | 典型例子 |
| ------ | -------- | -------- | -------- |
| **network** | **网线层失败**：请求根本没拿到 HTTP 响应 | `name=FetchError`，或消息精确匹配 `Failed to fetch` / `Load failed` / `NetworkError…`；且**不是**主动 `captureStructuredError` | 离线、DNS 挂、CORS preflight 挂、扩展拦请求 |
| **api_5xx** | **已经打到 API，对方回了 5xx** | `statusCode` 500–599，或 message 含 `\b500\|502\|503\|504\b` | BFF / order-api 返回 503 |
| **api_timeout** | **有请求语义，但超时/被中止** | `AbortError`，或 message 含 `timeout` / `ETIMEDOUT` / `timed out` | 客户端 abort、网关超时 |
| **app_error** | **业务代码主动上报** | `isExplicitCapture`（`mechanism.handled=true` + `type=generic`，即走了 `captureStructuredError`） | catch 后主动 `captureStructuredError(...)` |

怎么记：

1. **network** = 连合法 HTTP 状态都没有（断在传输层）；
2. **api_5xx** = 已有 HTTP，且是 5xx；
3. **api_timeout** = 超时/Abort，不是「对方回了 5xx」；
4. **app_error** = 不是启发式猜的，是你主动 capture；但若同一次事件里已能判出 5xx/timeout，**仍优先 api_***（HTTP 信号更准）。

补充：`Failed to fetch (evil.com)` 若域名既非自有（`castlery.com`）也非 critical 三方（Stripe/DY/Algolia…），会先被 **third_party** 吃掉，进不了 network。

```mermaid
flowchart TD
  A[客户端错误] --> B{浏览器原生 Failed to fetch<br/>且非主动 capture?}
  B -->|是| N[network]
  B -->|否| C{statusCode 5xx<br/>或消息含 500/502/503/504?}
  C -->|是| A5[api_5xx]
  C -->|否| D{AbortError / timeout 文案?}
  D -->|是| AT[api_timeout]
  D -->|否| E{captureStructuredError 主动上报?}
  E -->|是| AE[app_error]
  E -->|否| F[继续 js_fatal / unclassified…]
```

#### 服务端怎么区分 upstream_5xx / upstream_timeout / app_error

服务端没有 hydration / third_party / network；5xx/timeout 语义改成「**我方 Node 调上游**」：

| Bucket | 人话语义 | 判定信号 |
| ------ | -------- | -------- |
| **upstream_5xx** | 上游（order-api / 支付网关等）回了 5xx | 同客户端：`statusCode` 5xx 或消息含 500/502/503/504 |
| **upstream_timeout** | 调上游超时/Abort | 同客户端：`AbortError` / timeout 文案 |
| **app_error** | Server Action / 服务端业务主动 `captureStructuredError` | `isExplicitCapture` |

对照：客户端 `api_*` ↔ 服务端 `upstream_*`，信号相同，只是服务端强调「我们调别人」。
...
- **network 在 third_party 之后**：落进 network 的只剩自有域或关键三方的 fetch 失败；
- **客户端 app_error 先于 js_fatal**：`captureStructuredError(new TypeError(...))` 归 app_error——主动上报意图优先；但 HTTP 5xx/timeout 又优先于上报意图（主动报的 5xx 仍归 api_5xx）；
- **Hydration 放第一**：不是因为它最严重，而是判别式最准、最窄，否则会被更宽规则吞掉。

业务代码只负责带 domain/pageType 等上下文，不重复实现分桶。告警规则按 bucket + domain + page_type 组合过滤，而不是所有错误一股脑报警。

**分类器的判定链（客户端完整版，先匹配先赢）：**
...
我们的分桶收敛在 beforeSend 的一个分类器里，产出 error_bucket 和置信度两个 tag。客户端八类里最容易混的是：network 是「没拿到 HTTP」的网线失败，api_5xx 是「有 HTTP 且是 5xx」，api_timeout 是超时/Abort，app_error 是业务主动 captureStructuredError——但若已能判出 5xx/timeout 仍归 api_*。服务端没有 network，对应改叫 upstream_5xx/upstream_timeout。分类有严格优先级，比如第三方脚本先于 api_5xx，避免 Stripe 域的 5xx 污染我们自己的接口统计。分桶是告警路由的基础——告警按 bucket 加业务域组合，不是一刀切。
