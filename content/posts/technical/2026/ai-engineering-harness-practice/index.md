...
| **L1 规范** | `docs/ai-specs/` | 领域权威规范（可观测性、告警分桶等） | Skills 按需引用 |
...
2. **Commands** — `dev` / `build` / `test` / 领域专用命令（如可观测性 lint、Sentry E2E）
3. **Project Structure** — `apps/`、`libs/modules/`、`libs/shared/` 分层
...
├── alert-harness/        ← 团队专属：Sentry 分诊 + 自动修复
...
| Skill | 触发场景 | 价值 |
| --- | --- | --- |
| **alert-harness** | `/alert-harness`、Sentry 分诊、error_bucket 合规 | 连接 MCP + 规范 + 可选自动修复链 |
...
流程概要：加载 `docs/ai-specs/observability/` → Sentry MCP 拉 issue → 对照 13 条告警矩阵 → 可选修复循环。

Claude **Stop Hook** 在会话结束时跑 `stop-review.sh`，对本次改动做可观测性合规扫描——把「记得检查」变成「停不下来就不放行」。
...
| **observability-self-check** | 改可观测性路径后，最多 3 轮 `pnpm lint:observability`，失败则 ESCALATE |

observability-self-check 的验证环：

```
完成编辑 → lint:observability → 0 违规则结束
...
| 文件 | alwaysApply | 内容 |
| --- | --- | --- |
| **project-rules** | 是 | Monorepo 结构、Fortress 组件规范、ESLint 边界、Storybook 约定 |
...
- **permissions.allow** — 白名单 bash（git、lint 脚本、alert-harness scripts）
- **hooks.Stop** — 会话结束触发 `stop-review.sh`

### 6.2 Evals

`.claude/evals/` 存放 alert-harness 等 Skill 的评测用例与脚本，用于变更 Skill 后回归「AI 是否仍按流程执行」。
...
| # | 组件 | 回答问题 | 可观测性实例 |
| --- | --- | --- | --- |
| 1 | Spec | 正确长什么样？ | `docs/ai-specs/observability/INDEX.md` + 5 篇 spec |
| 2 | Static | 编写时能发现吗？ | `eslint-plugin-observability`（8 条 AST 规则） |
...
└── tech-doc-migration/     ← 企业文档 → 个人博客的去企业化规范

docs/joyboy/                ← 工程资料镜像（含完整 AGENTS/Skills 参考）
```

博客不写企业 `AGENTS.md`，但 **迁移 Skill 即宪法**：去品牌、去真实路径、人称个人化、保留技术细节。样式 Skill 则约束 `lib/brand.ts` ↔ `/brand` 展示页同步。
...
| 知识地图 | [工程知识图谱](/posts/joyboy-knowledge-map/) | 资料索引，含可观测性/规范方向 |
