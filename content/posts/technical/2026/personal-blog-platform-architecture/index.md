...
tags:
  ...
  - GitHub Pages
status: draft
excerpt: 复盘我把个人博客收成 monorepo 之后的结构：本地 CMS 与 SQLite 支撑日常发布，前台仍静态导出到 GitHub Pages；同时预留 PostgreSQL、Content API 与 ISR，但生产尚未切流。
---
...
IDE 里如果只改了正文，不必导入。一旦改了 frontmatter、或是完全手建的 Markdown，必须跑文件优先的导入，否则生产构建只会告警「库和文件不一致」，新文章不会出现在站点上。
...
我把博客收成一个小平台，但日常仍按静态站来用：本机管理端写稿，SQLite 记住状态，Markdown 保住正文，推到主分支后整站重建。仓库里另一条路——数据库作为运行时真源、前台可静态可 ISR——已经可以在本地复现，只是还没获得「拆掉旧路」的资格。
