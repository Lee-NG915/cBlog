...
当时前端支持 6+ 种支付渠道——Stripe 卡支付、Apple Pay、PayPal 弹窗、钱包类跳转支付、分期支付等。每种渠道的交互模型完全不同，但代码都堆在一个 500 行的支付页组件里：创建订单、初始化支付、调 SDK、处理回调、错误回滚全混在一起。新增一个支付商，要理解并改动核心 UI，回归成本越来越高。
...
这篇对应 Checkout/Payment、支付状态机、幂等、Server Action 和策略模式。阅读时可以按「创建订单 → 初始化支付 → 用户确认 → capture → callback/rollback」串起主流程，再理解为什么 UI 只消费 ActionSchema、渠道差异放在策略层。
...
├── actions/         Server Action（BFF(Backend-for-Frontend 中间层) 入口）
...
## Server Action 作为 BFF(Backend-for-Frontend 中间层) 入口
...
### 为什么不把支付逻辑放 BFF(Backend-for-Frontend 中间层) 全托管
...
4. **Server Action 做 BFF(Backend-for-Frontend 中间层)**——安全、追踪、组合根装配
