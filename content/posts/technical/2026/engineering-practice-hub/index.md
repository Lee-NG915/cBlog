...
tags:
  ...
  - Index
status: published
excerpt: 把多年电商前端工程实践整理成可导航的索引：分方向、分场景说明「为什么选当前方案」，并链到已发布的个人技术笔记。
---
...
这篇博文是在线版的导航目录：每条记录说明我当时的技术判断，**已整理成个人笔记的直接点文章链接**；还在打磨的会标为草稿或整理中。
...
| 项目规范（AGENTS）      | 把命名、分层、埋点红线写进 AI 可读规范，减少「每次口头对齐」                                 | 📂 整理中 | —                                                                     |
...
| PLP 技术方案         | 搜索列表页是流量入口，SearchKit + InstantSearch 的组合在定制性和交付速度之间比较平衡 | 📂 整理中 | —                                                                     |
| PLP 优先于 CLP       | 分类页和列表页的 SEO / 投放价值不同，资源分配要有优先级而不是平行推进                | 📂 整理中 | —                                                                     |
| PDP 数据分桶优化     | 商品详情页接口太重时，按展示区块分桶请求，比一次性拉全量更容易控 LCP                 | 📂 整理中 | —                                                                     |
| 搜索结果缓存         | 热门搜索词缓存可以显著降 ES 压力，但要处理好失效和个性化冲突                         | 📂 整理中 | —                                                                     |
| 本地开发性能         | 大 Monorepo 本地慢会直接影响迭代意愿，值得单独做一轮优化计划                         | 📂 整理中 | —                                                                     |
...
| 代码格式化规范             | Prettier + ESLint 统一格式，比靠 code review 抓风格便宜得多                     | 📂 整理中 | —                                                              |
...
| 支付架构重构 ADR        | 把 initiate / confirm / callback 拆清楚，比在一个 hook 里写完所有逻辑更易测试 | 📂 整理中 | —                                                         |
| Stripe Payment Element  | 卡支付走 Payment Element，Express 支付单独 slot，避免重复入口                 | 📂 整理中 | —                                                         |
| Stripe Express Checkout | Apple Pay / Google Pay / Link 用独立 Element，和卡支付共用后端 pipeline       | 📂 整理中 | —                                                         |
| Stripe Link UI 约束     | 第三方组件的样式定制有硬边界，提前写进方案比上线后才发现省事                  | 📂 整理中 | —                                                         |
| SPL 分期支付            | 区域性支付方式要隔离在 checkout 模块内，不能污染通用支付流程                  | 📂 整理中 | —                                                         |
| 延保（Warranty）        | 附加售卖是独立业务域，有自己的校验规则和架构边界                              | 📂 整理中 | —                                                         |
...
| 事件页模板       | 每个重要事件一份契约：含义、触发时机、字段、测试方案                    | 📂 整理中 | —                                                                    |
| Checkout 埋点    | 结账漏斗事件最容易「过早触发」，契约里要写清楚非目标场景                | 📂 整理中 | —                                                                    |
| 交易类埋点       | 支付成功 / 失败 / 取消要区分系统行为与用户行为                          | 📂 整理中 | —                                                                    |
| 多渠道映射       | GA / Meta CAPI / Klaviyo 等各有 payload 差异，trigger 层做转换          | 📂 整理中 | —                                                                    |
| 埋点变更操作规范 | 新增 / 修改 / 删除事件要走 Diff Report，避免 silent break               | 📂 整理中 | —                                                                    |
...
| API 错误码使用    | 业务错误码要和 UI 文案、埋点、Sentry tag 对齐，避免三方各写各的       | 📂 整理中 | —                                                              |
| 回退购物车错误    | 结账失败后的回退路径是高频客诉点，错误提示要 actionable               | 📂 整理中 | —                                                              |
...
| 主题               | 我的判断                                                                          | 笔记状态  | 链接 |
| ------------------ | --------------------------------------------------------------------------------- | --------- | ---- |
| PLP 排序与点击反馈 | 排序策略变更要能通过埋点验证效果，不能只看 CTR 单指标                             | 📂 整理中 | —    |
| Sale Page 方案     | 促销页是运营高频场景，模板化比每次开发新页面划算                                  | 📂 整理中 | —    |
| 三方服务集成总览   | 先画清楚依赖关系，再决定哪些走 BFF(Backend-for-Frontend 中间层)、哪些走客户端 SDK | 📂 整理中 | —    |
...
| Skills 体系             | 社区 skill 优先，团队专属规范才自建；源文件 + 软链双轨兼容 Cursor 和 Claude Code                        | 📂 整理中 | —                                                        |
| React 最佳实践 Skill    | Vercel 的 50+ 条规则按场景分类，适合 code review 和 AI 生成代码时做约束                                 | 📂 整理中 | —                                                        |
| Sentry Issue 修复 Skill | 线上 issue 处理有固定流程：复现 → 分桶 → 修复 → 验证                                                    | 📂 整理中 | —                                                        |
| PR 提交规范             | 分支名关联任务、PR 模板固定章节，减少 reviewer 的信息搜集成本                                           | 📂 整理中 | —                                                        |
| 本地 HTTPS 开发         | CMS 预览、支付回调等场景必须要 HTTPS 本地环境                                                           | 📂 整理中 | —                                                        |
...
| 主题                          | 场景                                                       | 笔记状态  | 链接 |
| ----------------------------- | ---------------------------------------------------------- | --------- | ---- |
| 购物车埋点边界重构            | listener 越界读取 store 补字段，导致埋点逻辑和业务逻辑耦合 | 📂 整理中 | —    |
| 价格展示 Server Function 报错 | RSC 边界下服务端函数渲染异常                               | 📂 整理中 | —    |
| 错误码清理                    | 历史遗留错误码冗余，影响告警和用户体验                     | 📂 整理中 | —    |
| 线上故障修复复盘              | 一次 Web 端线上故障的完整修复记录                          | 📂 整理中 | —    |
